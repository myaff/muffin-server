...
export class TrackingController {
  ...
  @Post()
  create(@Request() req, @Body() createTrackingDto: Omit<CreateTrackingDto, 'user'> | Omit<CreateTrackingDto, 'user'>[]) {
    const trackings = isArray(createTrackingDto) ? createTrackingDto : [createTrackingDto];
  ...
  @Patch(':id')
  update(@Request() req, @Param('id') id: string, @Body() updateTrackingDto: UpdateTrackingDto) {
  ...
  getTransformedOptions(query: unknown) {
    ...
    const dateOptions = {
      dateFrom: q.dateFrom && isDateString(q.dateFrom)
        ? q.dateFrom
        : null,
      dateTo: q.dateTo && isDateString(q.dateTo)
        ? q.dateTo
        : null,
    };
